...
<head>
  ...
  <style>
    :root {
      ...
      --info: #38bdf8;
      --danger: #ef4444;
      --focus-ring: rgba(134, 239, 172, .5);
      --focus-glow: rgba(34, 197, 94, .18);
    ...
    * {
    ...
    .workspace {
      ...
      background:
        ...
        radial-gradient(circle at bottom left, rgba(134, 239, 172, 0.06), transparent 30%),
        var(--bg)
    }
    ...
    .canvas-card.focus-card {
      box-shadow: 0 0 0 1px rgba(134, 239, 172, .32), var(--shadow);
      transform: none
    }
    ...
    .list-row.active {
      ...
      box-shadow: 0 0 0 1px rgba(134, 239, 172, .18), 0 14px 30px rgba(34, 197, 94, .12);
      transform: translateY(-1px)
    }
...
<body>
  <header>
    <div>
      <h1>Breadth First Search <span>Dashboard</span></h1>
      <p>Match the sorting-tech layout: side debug + tabs for animation, theory, and pseudocode.</p>
    </div>
    ...
    <aside class="left-panel">
      ...
      <section class="panel-section">
        ...
        <div class="summary-grid">
          ...
          <div class="metric"><div class="metric-label">Queue</div><div class="metric-value" id="queueSize">0</div></div>
      ...
      <section id="animation" class="tab-panel visualizer-panel active">
        ...
        <section class="viz-area">
          <article class="canvas-card" id="graphCard">
            <div class="card-head">
              <div>
                <div class="card-title">Graph + BFS Tree</div>
                <div class="card-subtitle graph-status" id="statusText">Load a sample and start BFS to see how the queue drives traversal order.</div>
              </div>
            </div>
        ...
        <section class="bottom-row">
          <article class="footer-card">
            ...
            <p id="insightText">BFS visits the graph level by level, so the queue always stores the frontier of vertices waiting to be processed.</p>
          </article>
        </section>
      </section>
    ...
    <section id="theory" class="tab-panel">
      <section class="theory-grid">
        <article class="theory-card">
          <h3>What BFS Does</h3>
          <ul>
            <li>BFS starts from a source vertex and explores all neighbors first.</li>
            <li>Then it moves to the next layer of neighbors, which is why it is called breadth first.</li>
            <li>The queue guarantees vertices are processed in discovery order.</li>
          </ul>
        </article>
        <article class="theory-card">
          <h3>Why The Queue Matters</h3>
          <ul>
            <li>When a new vertex is discovered, it is pushed to the back of the queue.</li>
            <li>The oldest waiting vertex is removed from the front.</li>
            <li>This first-in, first-out behavior creates level-by-level traversal.</li>
          </ul>
        </article>
        <article class="theory-card">
          <h3>Where BFS Is Used</h3>
          <ul>
            <li>Shortest path in unweighted graphs.</li>
            <li>Level-order exploration.</li>
            <li>Connected component discovery.</li>
            <li>Web crawling and network broadcasting style problems.</li>
          </ul>
        </article>
        <article class="theory-card">
          <h3>Complexity</h3>
          <ul>
            <li>With an adjacency list, BFS runs in <code>O(V + E)</code>.</li>
...
while queue is not empty
    ...
    for each neighbor of node
        ...
        <article class="theory-card">
          <h3>C Code Sketch</h3>
          <pre><code>void bfs(Node* graph[], int start, int V) {
    int visited[100] = {0};
    ...
    while (front < rear) {
        ...
        <article class="theory-card">
          <h3>Step Mapping</h3>
          <p>The animation follows the same rhythm every time: initialize the start vertex, dequeue the front item, inspect neighbors, enqueue every undiscovered neighbor, and stop when the queue becomes empty.</p>
        </article>
        <article class="theory-card">
          <h3>Teaching Note</h3>
          <p>BFS marks vertices when they are enqueued, not when they are dequeued. That prevents the same vertex from entering the queue multiple times.</p>
        </article>
      </section>
    </section>
  </div>
  ...
  <script>
    const FLOW_STEPS = [
      'Initialize the start vertex and place it in the queue.',
      'Dequeue the next vertex to process.',
      'Inspect each adjacent vertex of the current node.',
      'Enqueue every undiscovered neighbor.',
    ...
    function makeSnapshot(type, message, adj, queue, discovered, processed, activeVertex = null, activeEdge = null, treeEdges = []) {
      return {
        type,
        message,
        queue: [...queue],
    ...
    function buildActions(sample, start) {
      ...
      const treeEdges = [];
      const actions = [
        makeSnapshot('start', `Start BFS from ${start}. Mark it discovered and enqueue it.`, adj, queue, discovered, processed, start, null, treeEdges)
      ];

      while (queue.length) {
        const node = queue.shift();
        processed.push(node);
        actions.push(makeSnapshot('dequeue', `Dequeue ${node} and process it.`, adj, queue, discovered, processed, node, null, treeEdges));

        for (const neighbor of adj[node]) {
          const seen = discovered.has(neighbor);
          actions.push(makeSnapshot('inspect', seen ? `Inspect edge ${node}-${neighbor}. ${neighbor} was already discovered, so skip it.` : `Inspect edge ${node}-${neighbor}. ${neighbor} is new, so discover and enqueue it.`, adj, queue, discovered, processed, node, [node, neighbor], treeEdges));
          if (!seen) {
            discovered.add(neighbor);
            queue.push(neighbor);
            treeEdges.push([node, neighbor]);
            actions.push(makeSnapshot('enqueue', `Mark ${neighbor} discovered and enqueue it from ${node}.`, adj, queue, discovered, processed, neighbor, [node, neighbor], treeEdges));
          }
        }
      }

      actions.push(makeSnapshot('complete', `BFS complete. Final order: ${processed.join(' -> ')}.`, adj, queue, discovered, processed, null, null, treeEdges));
      return actions;
    }
    ...
    function loadSample(key, start, resetMessage = true) {
      ...
      render();
      if (resetMessage) {
        ui.statusText.textContent = `Start BFS from ${state.start}. Mark it discovered and enqueue it.`;
      }
    }
    ...
    function getActiveStepIndex(type) {
      if (type === 'start') return 0;
      if (type === 'dequeue') return 1;
      if (type === 'inspect') return 2;
      if (type === 'enqueue') return 3;
      return 4;
    }

    function getActiveCodeLines(type) {
      if (type === 'start') return [0, 1];
      if (type === 'dequeue') return [2, 3, 4];
      if (type === 'inspect') return [5, 6];
      if (type === 'enqueue') return [7, 8];
      return [2];
    }

    function renderSteps(type) {
      const activeIndex = getActiveStepIndex(type);
    ...
    function renderActiveCode(type) {
      const activeLines = new Set(getActiveCodeLines(type));
    ...
    function getFocusedCards(shot) {
      ...
      if (shot.type === 'dequeue') return [ui.graphCard, ui.orderCard];
      if (shot.type === 'inspect') return [ui.graphCard, ui.adjacencyCard];
      if (shot.type === 'enqueue') return [ui.frontierCard, ui.adjacencyCard];
      if (shot.type === 'complete') return [ui.orderCard];
      return [ui.graphCard];
    }

    function getAdjacencyFocusVertex(shot) {
      if ((shot.type === 'inspect' || shot.type === 'enqueue') && shot.activeEdge) return shot.activeEdge[0];
    ...
    function render() {
      ...
      ui.visitedCount.textContent = `${shot.processed.length} / ${state.sample.vertices.length}`;
      ui.queueSize.textContent = String(shot.queue.length);
      ...
      ui.insightText.textContent = shot.type === 'enqueue'
        ? `${shot.activeVertex} enters the queue now, so BFS will process it after all earlier discovered vertices.`
        : shot.type === 'dequeue'
          ? `${shot.activeVertex} is being processed now. BFS explores all of its neighbors before moving deeper.`
          : shot.type === 'inspect'
            ? `BFS checks neighbors one by one, but only undiscovered vertices are added to the queue.`
            : shot.type === 'complete'
              ? `The queue is empty, so BFS has finished exploring every reachable vertex from ${state.start}.`
              : `BFS visits the graph level by level, so the queue stores the frontier of vertices waiting to be processed.`;
      renderSteps(shot.type);
      renderActiveCode(shot.type);
      renderLogs();
      ...
      const discoveredSet = new Set(shot.discovered);
      const processedSet = new Set(shot.processed);
      const nodes = state.sample.vertices.map(v => {
        const [x, y] = state.sample.positions[v];
        const active = v === shot.activeVertex;
        const processed = processedSet.has(v);
        const discovered = discoveredSet.has(v);
        const fill = active ? '#f59e0b' : processed ? '#22c55e' : discovered ? '#38bdf8' : '#1a2333';
        const stroke = active ? '#fde68a' : processed ? '#86efac' : discovered ? '#7dd3fc' : '#4f6287';
        const text = active ? '#1f1303' : discovered || processed ? '#041522' : '#e2e8f0';
      ...
      ui.queueBox.innerHTML = `<div class="row-strip">${shot.queue.length ? shot.queue.map(v => `<div class="node-pill ${v === shot.activeVertex ? 'active' : ''}">${v}</div>`).join('') : '<div class="empty">empty</div>'}</div>`;
      ui.orderBox.innerHTML = `<div class="row-strip">${shot.processed.length ? shot.processed.map(v => `<div class="order-pill ${v === shot.activeVertex ? 'active' : ''}">${v}</div>`).join('') : '<div class="empty">none</div>'}</div>`;
