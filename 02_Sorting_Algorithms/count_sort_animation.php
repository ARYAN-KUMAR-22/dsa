...
<head>
  ...
  <title>Counting Sort - Interactive Visualizer</title>
  <link
  ...
  <style>
    ...
    * {
    ...
    header {
      ...
      background: linear-gradient(90deg, #0f1600, #0d1117);
      flex-shrink: 0;
    }
    ...
    .workspace {
      ...
      overflow: hidden;
      background:
        radial-gradient(circle at top right, rgba(132, 204, 22, 0.1), transparent 28%),
        radial-gradient(circle at bottom left, rgba(190, 242, 100, 0.07), transparent 30%),
        var(--bg);
    }
    ...
    .buffer-cell.active {
      ...
      border-color: var(--accent);
      box-shadow: 0 0 16px rgba(132, 204, 22, 0.16);
    }
...
<a href="../dashboard.php?view=02_Sorting_Algorithms/index.php" style="position:fixed; top:20px; left:20px; z-index:9999999; padding:10px 20px; background:#ef4444; color:white; font-family:'Inter', sans-serif; font-size:14px; text-decoration:none; border-radius:8px; box-shadow:0 8px 15px rgba(0,0,0,0.2); font-weight:600; display:flex; align-items:center; gap:8px; transition:transform 0.2s;">&larr; Back to Dashboard</a>
  ...
  <header>
    <div>
      <h1><span>Counting Sort</span> Visualizer</h1>
      <p>Track frequencies, prefix sums, and stable output placement step by step.</p>
    </div>
      ...
      <section class="panel-section">
        ...
        <div class="summary-grid">
          <div class="metric">
            ...
            <div class="metric-value" id="countOps">0</div>
          </div>
          <div class="metric">
          ...
          <div class="metric">
            ...
            <div class="metric-value" id="copyWrites">0</div>
          </div>
        </div>
      </section>
    ...
    <main class="workspace">
      ...
      <section id="tab-visualizer" class="tab-panel visualizer-panel active">
        <section class="control-row">
          <label class="field">
            <span>Input</span>
            <input id="inputArray" type="text" value="4, 2, 2, 8, 3, 3, 1, 0"
          ...
          <article class="canvas-card">
            <div class="card-head">
              <div>
                ...
                <div class="card-subtitle" id="statusText">Ready to simulate Counting Sort.</div>
              </div>
              <div class="legend">
                <span class="legend-item"><span class="legend-swatch" style="background:#f59e0b;"></span> Current input</span>
                <span class="legend-item"><span class="legend-swatch" style="background:#84cc16;"></span> Active count/output slot</span>
                <span class="legend-item"><span class="legend-swatch" style="background:#f43f5e;"></span> Stable placement</span>
            ...
            <div class="buffer-grid">
              <div class="buffer-panel">
                <div class="buffer-head">
                  <div class="buffer-title">Count Array</div>
                  <div class="buffer-subtitle" id="countStatus">Counts start at zero for every value in range.</div>
                </div>
              ...
              <div class="buffer-panel">
                <div class="buffer-head">
                  ...
                  <div class="buffer-subtitle" id="outputStatus">Output fills from right to left for stable sorting.</div>
                </div>
        ...
        <section class="bottom-row">
          <article class="footer-card">
            ...
            <div class="badge-row">
              ...
              <span class="badge">Worst: O(n + k)</span>
          ...
          <article class="footer-card">
            ...
            <p class="status-line" id="insightText">
              <strong>Counting Sort</strong> counts how often each value appears, turns those counts into positions, and
      ...
      <section id="tab-theory" class="tab-panel">
        <div class="theory-grid">
          <article class="theory-card">
            ...
            <ul>
              <li>Create a count array that records how many times each value appears.</li>
              <li>Convert counts into prefix sums so each value knows its final ending position.</li>
              <li>Walk the input from right to left to keep equal values in stable order.</li>
              <li>Place each value directly into the correct output slot, then decrease its count.</li>
              <li>Copy the output array back to finish with a sorted result.</li>
            </ul>
          </article>
          ...
          <article class="theory-card">
            <h3>Pseudo Logic</h3>
            <ul>
              <li><code>count = array(maxValue + 1, 0)</code></li>
              <li><code>for value in a: count[value]++</code></li>
              <li><code>for i = 1 to maxValue: count[i] += count[i - 1]</code></li>
              <li><code>for i = n - 1 down to 0</code></li>
              <li><code>output[count[a[i]] - 1] = a[i]</code></li>
              <li><code>count[a[i]]--</code></li>
            </ul>
          </article>
          ...
          <article class="theory-card">
            ...
            <ul>
              <li>Very fast when the key range <code>k</code> is small compared with the number of items <code>n</code>.</li>
              <li>Useful as a building block inside radix sort and other integer-key algorithms.</li>
              <li>Great when stable ordering matters and values are small non-negative integers.</li>
            </ul>
          </article>
          ...
          <article class="theory-card">
            <h3>Key Tradeoffs</h3>
            <ul>
              <li>It is not comparison-based, so it only works well for restricted integer ranges.</li>
              <li>Space usage grows with both the input size and the value range.</li>
              <li>If the maximum value is very large, counting sort can waste memory and lose its advantage.</li>
            </ul>
          </article>
        </div>
      </section>
      ...
      <section id="tab-comparison" class="tab-panel">
        <div class="theory-grid">
          <article class="theory-card">
            <h3>Counting Sort At a Glance</h3>
            <ul>
              <li>Counting Sort is not comparison-based and runs in <code>O(n + k)</code> when the value range is small.</li>
              <li>It is stable, but it needs extra memory for both the count array and the output array.</li>
              <li>Its speed depends less on input order and more on how large the key range is.</li>
            </ul>
          </article>
          ...
          <article class="theory-card">
            ...
            <ul>
              <li>When <code>k</code> is small, Counting Sort can beat comparison sorts because it does not rely on pairwise ordering tests.</li>
              <li><strong>Merge Sort</strong> and <strong>Quick Sort</strong> are more flexible because they work for general comparable values.</li>
              <li>If the range becomes huge, Counting Sort loses its memory and speed advantage quickly.</li>
            </ul>
          </article>

          <article class="theory-card">
            <h3>Compared With Radix Sort</h3>
            <ul>
              <li><strong>Radix Sort</strong> uses counting-style passes repeatedly to handle larger integers digit by digit.</li>
              <li>Counting Sort is simpler when the full value range is already small enough.</li>
              <li>Radix Sort is more scalable for bounded-width numbers, but Counting Sort is the more direct tool.</li>
            </ul>
          </article>
          ...
          <article class="theory-card">
            <h3>Best Fit</h3>
            <ul>
              <li>Choose Counting Sort for small non-negative integer ranges where stability matters.</li>
              <li>It is especially useful as a building block inside Radix Sort.</li>
              <li>If the keys are wide, sparse, or not integer-like, use a different family of sort.</li>
            </ul>
          </article>
        </div>
      </section>
  ...
  <script>
    const defaultArray = [4, 2, 2, 8, 3, 3, 1, 0];
    const maxBars = 14;
    const minValue = 0;
    const maxValue = 12;
    ...
    const stepsTemplate = [
      ...
      'Increment the matching count slot.',
      'Convert counts into prefix sums.',
      'Place values into output using stable positions.',
      'Copy the output array back to finish sorting.'
    ];

    const pseudoCode = [
      'count = array(maxValue + 1, 0)',
      'for value in a: count[value]++',
      'for i = 1 to maxValue: count[i] += count[i - 1]',
      'for i = n - 1 down to 0',
    ...
    const ui = {
      ...
      copyWrites: document.getElementById('copyWrites'),
    ...
    function init() {
      renderSteps();
      renderPseudoCode();
      loadArray(defaultArray, 'Loaded default Counting Sort sample.');
      bindEvents();
    }
    ...
    function bindEvents() {
      ...
      ui.loadBtn.addEventListener('click', () => {
        ...
        if (!parsed.length) {
          pushLog('warn', 'Invalid input. Enter up to 14 comma separated integers between 0 and 12.');
          return;
        }
    ...
    function loadArray(arr, message) {
      ...
      updateStatus('Ready to simulate Counting Sort.');
      updateInsight('counts how often each value appears, turns those counts into positions, and writes the output directly.');
      updateCountStatus('Counts start at zero for every value in range.');
      updateOutputStatus('Output fills from right to left for stable sorting.');
      renderAll();
    }
    ...
    function buildActions(source) {
      ...
      const count = Array.from({ length: largest + 1 }, () => 0);
      const output = Array.from({ length: n }, () => null);
      ...
      const finalArray = output.map(value => value ?? 0);

      actions.push({
        type: 'copyBack',
        finalArray: [...finalArray],
        countSnapshot: [...count],
        outputSnapshot: [...finalArray],
        line: 6,
        step: 4,
        copyAmount: finalArray.length,
        sorted: finalArray.map((_, index) => index)
      });
    ...
    function renderMetrics() {
      ui.countOps.textContent = state.countOps;
      ui.prefixOps.textContent = state.prefixOps;
      ui.outputWrites.textContent = state.outputWrites;
      ui.copyWrites.textContent = state.copyWrites;
    }
