...
<head>
  ...
  <title>Insertion Sort - Interactive Visualizer</title>
  <link
  ...
  <style>
    ...
    * {
    ...
    header {
      ...
      background: linear-gradient(90deg, #1a1100, #0d1117);
      flex-shrink: 0;
    }
...
<a href="../dashboard.php?view=02_Sorting_Algorithms/index.php" style="position:fixed; top:20px; left:20px; z-index:9999999; padding:10px 20px; background:#ef4444; color:white; font-family:'Inter', sans-serif; font-size:14px; text-decoration:none; border-radius:8px; box-shadow:0 8px 15px rgba(0,0,0,0.2); font-weight:600; display:flex; align-items:center; gap:8px; transition:transform 0.2s;">&larr; Back to Dashboard</a>
  ...
  <header>
    <div>
      <h1><span>Insertion Sort</span> Visualizer</h1>
      <p>Track the current key, right shifts, and the growing sorted prefix.</p>
    </div>
  ...
  <div class="app-body">
    ...
    <main class="workspace">
          ...
          <article class="canvas-card">
            <div class="card-head">
              <div>
                ...
                <div class="card-subtitle" id="statusText">Ready to simulate Insertion Sort.</div>
              </div>
              <div class="legend">
                <span class="legend-item"><span class="legend-swatch" style="background:#f59e0b;"></span> Current key</span>
                <span class="legend-item"><span class="legend-swatch" style="background:#f97316;"></span> Comparing</span>
                <span class="legend-item"><span class="legend-swatch" style="background:#ef4444;"></span> Shifting</span>
        ...
        <section class="bottom-row">
          ...
          <article class="footer-card">
            ...
            <p class="status-line" id="insightText">
              <strong>Insertion Sort</strong> grows a sorted prefix by taking one key at a time and inserting it into
      ...
      <section id="tab-theory" class="tab-panel">
        <div class="theory-grid">
          <article class="theory-card">
            ...
            <ul>
              <li>The left part of the array is treated as a sorted prefix.</li>
              <li>The next unsorted element becomes the key to insert.</li>
              <li>Larger elements shift right until the key can be placed.</li>
            </ul>
          </article>
          ...
          <article class="theory-card">
            <h3>Pseudo Logic</h3>
            <ul>
              <li><code>for i = 1 to n - 1</code></li>
              <li><code>key = a[i]</code></li>
              <li><code>j = i - 1</code></li>
              <li><code>while j &gt;= 0 and a[j] &gt; key</code></li>
              <li><code>a[j + 1] = a[j]</code></li>
              <li><code>a[j + 1] = key</code></li>
            </ul>
          </article>
          ...
          <article class="theory-card">
            ...
            <ul>
              <li>Very effective on small arrays and arrays that are already nearly sorted.</li>
              <li>Often used inside faster hybrid algorithms for tiny partitions.</li>
              <li>Its best case is linear, <code>O(n)</code>, when the input is already sorted.</li>
            </ul>
          </article>
          ...
          <article class="theory-card">
            <h3>Key Tradeoffs</h3>
            <ul>
              <li>Average and worst-case time are <code>O(n^2)</code>.</li>
              <li>It is stable, so equal values keep their relative order.</li>
      ...
      <section id="tab-comparison" class="tab-panel">
        <div class="theory-grid">
          <article class="theory-card">
            <h3>Insertion Sort At a Glance</h3>
            <ul>
              <li>Insertion Sort is <code>stable</code>, <code>in place</code>, and adaptive, with a best case of <code>O(n)</code>.</li>
              <li>Its worst case is still <code>O(n^2)</code>, so it is best for small or partially sorted inputs.</li>
              <li>It works by extending a sorted prefix one element at a time.</li>
            </ul>
          </article>

          <article class="theory-card">
            <h3>Compared With Bubble and Selection</h3>
            <ul>
              <li>Insertion Sort usually beats <strong>Bubble Sort</strong> on nearly sorted data because elements move directly to their place.</li>
              <li>It is also adaptive, unlike <strong>Selection Sort</strong>, which still scans the whole unsorted region every pass.</li>
              <li>Selection Sort performs fewer writes, but Insertion Sort is often the faster of the three in practice.</li>
            </ul>
          </article>

          <article class="theory-card">
            <h3>Compared With Shell, Merge, and Quick</h3>
            <ul>
              <li><strong>Shell Sort</strong> improves on insertion ideas by moving elements farther early with gaps.</li>
              <li><strong>Merge Sort</strong> and <strong>Quick Sort</strong> scale much better on large arrays.</li>
              <li>Insertion Sort still remains useful as a tiny-array helper inside faster hybrid sorts.</li>
            </ul>
          </article>
          ...
          <article class="theory-card">
            <h3>Best Fit</h3>
            <ul>
              <li>Choose Insertion Sort for very small arrays, almost-sorted arrays, or as a base case inside larger algorithms.</li>
              <li>It is a strong educational step up from Bubble Sort because it shows local shifting instead of repeated swapping.</li>
              <li>For large random arrays, prefer an <code>O(n log n)</code> algorithm.</li>
            </ul>
          </article>
        </div>
      </section>
  ...
  <script>
    const defaultArray = [34, 12, 55, 21, 8, 40, 29, 17];
    const maxBars = 14;
    const minValue = 5;
    const maxValue = 99;
    ...
    const stepsTemplate = [
      ...
      'Scan left through the sorted prefix.',
    ...
    function init() {
      renderSteps();
      renderPseudoCode();
      loadArray(defaultArray, 'Loaded default Insertion Sort sample.');
      bindEvents();
    }
    ...
    function loadArray(arr, message) {
      ...
      state.shiftCount = 0;
      state.passCount = 0;
      state.sortedIndices = arr.length ? [0] : [];
      state.highlight = [];
      state.shiftIndices = [];
      ...
      updateStatus(arr.length ? 'Ready to simulate Insertion Sort. The first element already forms a sorted prefix.' : 'Ready to simulate Insertion Sort.');
      updateInsight('Insertion Sort grows a sorted prefix by taking one key at a time and inserting it into the correct position.');
      renderAll();
    }
    ...
    function buildActions(source) {
      ...
      for (let i = 1; i < n; i++) {
        ...
        actions.push({ type: 'passStart', pass: i, key, index: i, line: 1, step: 0, sorted: sortedPrefix });
        actions.push({ type: 'selectKey', key, index: i, line: 2, step: 1, sorted: sortedPrefix });
        actions.push({ type: 'setPointer', key, j, line: 3, step: 1, sorted: sortedPrefix });
    ...
    function applyAction(action) {
      ...
      switch (action.type) {
        case 'passStart':
          ...
          updateStatus(`Pass ${action.pass}: choose key ${action.key} from index ${action.index}.`);
          updateInsight(`Insertion Sort now inserts ${action.key} into the sorted prefix on its left.`);
          pushLog('info', `Pass ${action.pass} started. Key is ${action.key} at index ${action.index}.`);
          break;

        case 'selectKey':
          state.selectedIndex = action.index;
          state.currentKey = action.key;
          updateStatus(`Holding key ${action.key} and preparing to scan left.`);
          updateInsight('The next unsorted value becomes the key. We conceptually hold it aside while looking for its correct place.');
          break;

        case 'setPointer':
          state.currentKey = action.key;
          updateStatus(`Start scanning left from index ${Math.max(action.j, 0)}.`);
          break;
        ...
        case 'compare':
          ...
          updateStatus(`Compare key ${action.key} with ${action.comparedValue} at index ${action.compareIndex}.`);
          updateInsight(`Comparison ${state.compareCount}: move left while elements are larger than the key.`);
          pushLog('info', `Compare key ${action.key} with value ${action.comparedValue} at index ${action.compareIndex}.`);
          break;
        ...
        case 'shift':
          ...
          state.currentKey = action.key;
          state.shiftCount += 1;
          updateStatus(`Shifted ${action.shiftedValue} from index ${action.from} to ${action.to}.`);
          updateInsight(`Shift ${state.shiftCount}: larger elements slide right to make room for the key ${action.key}.`);
          pushLog('warn', `Shifted ${action.shiftedValue} right. Array is now [${state.arr.join(', ')}].`);
          break;

        case 'moveLeft':
          state.currentKey = action.key;
          if (action.j >= 0) {
            state.highlight = [action.j];
            updateStatus(`Move left to continue checking where ${action.key} belongs.`);
          } else {
            updateStatus(`Reached the beginning. ${action.key} will be inserted at index 0.`);
          }
          break;
        ...
        case 'insert':
          ...
          state.selectedIndex = action.index;
          state.currentKey = action.key;
          updateStatus(`Inserted key ${action.key} at index ${action.index}.`);
    ...
    function updateInsight(text) {
      ...
      ui.insightText.innerHTML = `<strong>Insertion Sort</strong> ${text}`;
    }
