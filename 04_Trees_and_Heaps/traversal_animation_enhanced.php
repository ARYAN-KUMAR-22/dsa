...
<head>
  ...
  <title>Tree Traversals – Animated Visualizer</title>
  <link
...
<body>
  ...
  <script>
    ...
    function clearVisit() { const d = document.getElementById('visitOrder'); d.innerHTML = '<span style="color:#3a4060;font-size:.75rem;padding:4px 0">Running…</span>'; }
    ...
    async function runTraversal(type) {
      ...
      activeCol = info.col;
      resetColors(root); clearVisit();
      setSteps(info.steps, 0);
      log(`▶ Starting ${info.label}…`, 'info');
      const visited = [];

      if (type === 'inorder') { await inorder(root, visited); }
      else if (type === 'preorder') { await preorder(root, visited); }
      else if (type === 'postorder') { await postorder(root, visited); }
      else { await levelOrder(root, visited); }

      setSteps(info.steps, info.steps.length);
      log(`✅ ${info.label} complete: [${visited.join(' → ')}]`, 'success');
      running = false; clearSteps();
    }

    async function visit(nd, arr) {
      nd.col = activeCol; arr.push(nd.v); addVisit(nd.v);
      log(`Visiting: ${nd.v}`, 'info');
      await sleep(spd());
    }

    async function inorder(nd, arr) {
      if (!nd) return;
      setSteps(TRAV_INFO.inorder.steps, 0); await inorder(nd.l, arr);
      setSteps(TRAV_INFO.inorder.steps, 1); await visit(nd, arr);
      setSteps(TRAV_INFO.inorder.steps, 2); await inorder(nd.r, arr);
    }
    async function preorder(nd, arr) {
      if (!nd) return;
      setSteps(TRAV_INFO.preorder.steps, 0); await visit(nd, arr);
      setSteps(TRAV_INFO.preorder.steps, 1); await preorder(nd.l, arr);
      setSteps(TRAV_INFO.preorder.steps, 2); await preorder(nd.r, arr);
    }
    async function postorder(nd, arr) {
      if (!nd) return;
      setSteps(TRAV_INFO.postorder.steps, 0); await postorder(nd.l, arr);
      setSteps(TRAV_INFO.postorder.steps, 1); await postorder(nd.r, arr);
      setSteps(TRAV_INFO.postorder.steps, 2); await visit(nd, arr);
    }
    async function levelOrder(nd, arr) {
      const q = [nd]; let step = 0;
      while (q.length) {
        const cur = q.shift();
        setSteps(TRAV_INFO.level.steps, Math.min(step, 3)); step++;
        await visit(cur, arr);
    ...
    function doReset() {
      if (running) return;
      buildTree(); resetColors(root); clearVisit(); clearSteps();
      log('Tree reset to default', 'info');
    }
    function loadCustom() {
      if (running) return;
      root = null;
      const vals = [40, 20, 60, 10, 30, 50, 70, 5, 15, 25, 35];
      vals.forEach(v => { root = bstIns(root, v); });
      resetColors(root); clearVisit(); clearSteps();
      log('Loaded alternate tree (40-rooted, 11 nodes)', 'info');
    }
    ...
    window.runTraversal = runTraversal; window.doReset = doReset; window.loadCustom = loadCustom; window.showTab = showTab;
  </script>
