...
<body>
  <header>
    <div>
      <h1><span>Maximum Subarray</span> Sum - Kadane's Algorithm</h1>
      <p>Optimized O(n) algorithm to find the contiguous subarray with largest sum</p>
    </div>
    <div class="actions">
  ...
  <div class="app">
    <aside class="left">
      ...
      <section class="sec">
        ...
        <div class="metrics">
          ...
          <div class="metric"><div class="metric-label">Progress</div><div class="metric-value" id="metricProgress">0%</div></div>
      ...
      <section class="sec">
        ...
        <div class="box" style="font-size:.75rem;font-weight:700;color:var(--good)">O(n) Time<br/>O(1) Space</div>
      </section>
    ...
    <main class="work">
      <div class="tabs">
        <button class="tab active" data-tab="visualizer">🔍 Kadane's Execution</button>
      ...
      <div class="panel active" id="tab-visualizer">
        <div class="viz-container">
          <div class="viz-box">
            <div class="viz-head">💡 Algorithm Logic</div>
            <div class="viz-content" id="codeArea"></div>
          </div>
          <div class="viz-box">
            <div class="viz-head">📊 Array Visualization</div>
      ...
      <div class="panel" id="tab-learn">
        <div style="overflow-y:auto;padding-right:10px">
          <h3 style="color:var(--accent);margin:10px 0 6px;font-size:.95rem">Problem: Maximum Subarray Sum</h3>
          <div class="box">
            Find the contiguous subarray with the largest sum. Simple example: [-2, 1, -3, 4, -1, 2, 1, -5, 4] → Answer: 6 (subarray [4, -1, 2, 1])
          </div>

          <h3 style="color:var(--accent);margin:14px 0 6px;font-size:.95rem">Kadane's Algorithm Insight</h3>
          <div class="box">
            <strong>Key Idea:</strong> At each position, decide: keep adding to current sum OR start fresh?<br/><br/>
            • If current_sum becomes negative, reset it to 0<br/>
            • Track the maximum sum seen so far<br/>
            • Single pass through array = O(n)
          </div>

          <h3 style="color:var(--accent);margin:14px 0 6px;font-size:.95rem">Step-by-Step Logic</h3>
          <div class="box">
            1. For each element: current_sum = max(0, current_sum + element)<br/>
            2. Update max_sum = max(max_sum, current_sum)<br/>
            3. Continue to next element<br/>
            4. Return max_sum
          </div>

          <h3 style="color:var(--accent);margin:14px 0 6px;font-size:.95rem">Why So Fast?</h3>
          <div class="box">
            <strong>Time:</strong> O(n) — single pass<br/>
            <strong>Space:</strong> O(1) — only need 2 variables<br/>
            <strong>vs Brute Force:</strong> O(n²) — all subarrays<br/>
            <strong>Performance:</strong> 100x faster on n=1000
          </div>

          <h3 style="color:var(--accent);margin:14px 0 6px;font-size:.95rem">Visual Guide</h3>
          <div class="box">
            🟨 Orange: Current element being checked<br/>
            🟦 Blue: Current subarray in consideration<br/>
            🟩 Green: Best maximum subarray found<br/>
            🔴 Red: Negative values (affect sum)
          </div>
        </div>
      </div>
    </main>
  ...
  <script>
    ...
    const codeLines = [
      ...
      "    currentSum = max(0, currentSum + element)",
    ...
    async function runKadane() {
      ...
      for (let i = 0; i < state.arr.length; i++) {
        ...
        const old = state.currentSum;
        state.currentSum = Math.max(0, state.currentSum + state.arr[i]);
    ...
    function updateDisplay() {
      ...
      let statsHtml = `<div class="step-explanation">
        ...
        Current Sum: ${state.currentSum} | Max Sum: ${state.maxSum}<br/>
        ${state.currentStart <= state.maxEnd ? `<span style="color:var(--good)">✓ Best: [${state.maxStart}...${state.maxEnd}]</span>` : 'Searching...'}
      </div>`;
    ...
    function renderArray() {
      ...
      state.arr.forEach((val, i) => {
        ...
        if (i === state.idx) {
          classes += ' active';
        } else if (i >= state.currentStart && i <= state.idx) {
          classes += ' in-range';
        }
        
        if (i >= state.maxStart && i <= state.maxEnd) {
          classes += ' max';
        }
