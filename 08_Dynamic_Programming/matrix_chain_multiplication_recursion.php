...
<head>
  ...
  <title>Matrix Chain Multiplication - Recursion</title>
  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .stack-item{margin:0 10px 6px;padding:8px;border-radius:8px;border-left:3px solid var(--accent);background:rgba(96,165,250,.1);font-size:.7rem;font-family:"Fira Code",monospace}
    ...
    .code-line.passed{color:var(--muted);opacity:.6}
    ...
    .memo-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(28px,1fr));gap:2px;margin:4px 0}
    .memo-cell{width:28px;height:28px;display:flex;align-items:center;justify-content:center;border-radius:4px;font-size:.6rem;font-weight:700;border:1px solid var(--border);background: var(--surface);color:var(--muted)}
...
<body>
  <header>
    <div>
      <h1><span>Matrix Chain</span> Multiplication - Recursion</h1>
      <p>Live code execution visualization - watch step-by-step how the algorithm runs</p>
    </div>
    <div class="actions">
    ...
    <main class="work">
      ...
      <div class="panel active" id="tab-visualizer">
        <div class="viz-container">
          ...
          <div class="viz-box">
            <div class="viz-head">🧠 Call Stack & Memo</div>
      ...
      <div class="panel" id="tab-learn">
        <div style="overflow-y:auto;padding-right:10px">
          ...
          <div class="box">
            ...
            For n=4: ~15 calls<br/>For n=5: ~100+ calls<br/>
  ...
  <script>
    ...
    const state = {
      dims: [10, 30, 5, 60], callCount: 0, maxDepth: 0, result: 0, n: 0,
    ...
    async function buildTree(i, j, dims, depth = 0, lineLog = []) {
      state.callCount++;
      ...
      state.callStack[state.callStack.length - 1].result = minCost;
      state.computed.add(`[${i},${j}]=${minCost}`);
    ...
    function updateDisplay() {
      ...
      state.callStack.forEach((call, idx) => {
        stackHtml += `<div class="call-box">
          <div class="call-label">Frame ${idx + 1}: MCM(${call.i}, ${call.j})</div>
          <div class="call-content">${call.result !== null ? `Result: ${fmt(call.result)}` : 'Computing...'}</div>
        </div>`;
      });
