...
<head>
  ...
  <title>Longest Common Subsequence - Memoization</title>
...
<body>
  <header>
    <div>
      <h1><span>Longest Common</span> Subsequence - Memoization</h1>
    ...
    <main class="work">
      ...
      <div class="panel active" id="tab-visualizer">
        <div class="viz-container">
          ...
          <div class="viz-box">
            <div class="viz-head">💾 Cache & Stats</div>
            <div class="viz-content">
              <div id="statsDisplay"></div>
              <div style="margin-top:12px;padding-top:12px;border-top:1px solid var(--border)">
                <div style="font-size:.7rem;font-weight:700;color:var(--accent);margin-bottom:6px">Cached (i,j):</div>
      ...
      <div class="panel" id="tab-learn">
        <div style="overflow-y:auto;padding-right:10px">
          ...
          <div class="box">
            <strong>Cache Before Recursing!</strong> We check if LCS(i,j) already exists in memory. If yes, return it instantly (cache hit). If no, compute it and store for future use.
          </div>
          <h3 style="color:var(--accent);margin:14px 0 6px;font-size:.95rem">Key Improvement</h3>
          <div class="box">
            <strong>Polynomial Time!</strong><br/>
            • Recursion: ~O(2^n) exponential<br/>
            • Memoization: O(m×n) polynomial<br/>
            • Cache hits: Dramatic speedup visible in metrics
          </div>
          <h3 style="color:var(--accent);margin:14px 0 6px;font-size:.95rem">Top-Down DP</h3>
          <div class="box">
            Memoization is "top-down" recursion with caching. Start from big problem, break into smaller ones, cache results. Each (i,j) computed exactly once.
          </div>
        </div>
      </div>
    </main>
  ...
  <script>
    ...
    const state = {
      str1: "AGGTAB", str2: "GXTXAYB", callCount: 0, totalCalls: 0, cacheHits: 0, result: 0,
      running: false, memo: new Map(), currentLine: null, startTime: 0, speed: 1, currentCall: null
    };
    ...
    function updateDisplay() {
      ...
      let statsHtml = `<div class="stat-box">
        <div class="stat-label">Current Call: LCS(${state.currentCall?.i || '—'}, ${state.currentCall?.j || '—'})</div>
      ...
      if (state.memo.size > 0) {
        const sorted = Array.from(state.memo.entries()).sort((a, b) => {
