...
<head>
  ...
  <title>Maximum Subarray Sum - Dynamic Programming</title>
  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .array-viz{display:flex;gap:4px;margin:8px 0;flex-wrap:wrap}
    ...
    .dp-table{display:inline-flex;gap:4px;margin:8px 0;flex-wrap:wrap}
    .dp-cell{width:52px;height:52px;display:flex;align-items:center;justify-content:center;border-radius:8px;border:2px solid var(--border);background: var(--card);font-size:.7rem;font-weight:700;color:var(--text);transition:all .15s}
    .dp-cell.computed{background:linear-gradient(135deg,#22c55e,#16a34a);color:#0b1020;box-shadow:0 0 10px rgba(34,197,94,.5);border-color:#22c55e}
    .dp-cell.active{background:linear-gradient(135deg,#f59e0b,#d97706);color:#0b1020;box-shadow:0 0 16px rgba(245,158,11,.8);border-color:#f59e0b;transform:scale(1.1)}
    .dp-cell.header{background:rgba(96,165,250,.15);color:var(--accent3);font-size:.65rem;font-weight:800;border-color:var(--accent);width:48px;height:48px}
...
<body>
  <header>
    <div>
      <h1><span>Maximum Subarray</span> Sum - Dynamic Programming</h1>
      <p>DP recurrence with table visualization - O(n) with explicit state tracking</p>
    </div>
    <div class="actions">
      <a class="primary" href="../index.php">Home</a>
      <a href="max_subarray_brute_force.php">Brute Force</a>
      <a href="max_subarray_sum.php">Kadane's</a>
    </div>
  </header>
  ...
  <div class="app">
    <aside class="left">
      ...
      <section class="sec">
        ...
        <div class="metrics">
          ...
          <div class="metric"><div class="metric-label">DP[i]</div><div class="metric-value" id="metricDP">0</div></div>
          <div class="metric"><div class="metric-label">Max So Far</div><div class="metric-value" id="metricMax">0</div></div>
      ...
      <section class="sec">
        ...
        <div class="box" style="font-size:.75rem;font-weight:700;color:var(--good)">O(n) Time<br/>O(n) Space</div>
      </section>
    ...
    <main class="work">
      <div class="tabs">
        <button class="tab active" data-tab="visualizer">🔍 DP State Evolution</button>
      ...
      <div class="panel active" id="tab-visualizer">
        <div class="viz-container">
          <div class="viz-box">
            <div class="viz-head">📝 DP Recurrence</div>
            <div class="viz-content" id="codeArea"></div>
          </div>
          <div class="viz-box">
            <div class="viz-head">📊 Array & DP Table</div>
            <div class="viz-content" style="display:flex;flex-direction:column;gap:8px;overflow:auto">
              <div id="statsDisplay"></div>
              <div><strong style="color:var(--accent)">Input Array:</strong></div>
              <div class="array-viz" id="arrayDisplay"></div>
              <div><strong style="color:var(--accent)">DP Table (max ending at i):</strong></div>
      ...
      <div class="panel" id="tab-learn">
        <div style="overflow-y:auto;padding-right:10px">
          <h3 style="color:var(--accent);margin:10px 0 6px;font-size:.95rem">Dynamic Programming Approach</h3>
          <div class="box">
            Define: dp[i] = maximum sum of subarray <strong>ending at position i</strong><br/>
            Recurrence: dp[i] = max(arr[i], dp[i-1] + arr[i])<br/>
            Answer: max(dp[0], dp[1], ..., dp[n-1])
          </div>

          <h3 style="color:var(--accent);margin:14px 0 6px;font-size:.95rem">The Key Insight</h3>
          <div class="box">
            For each position, we have two choices:<br/>
            ✓ <strong>Start fresh:</strong> dp[i] = arr[i]<br/>
            ✓ <strong>Extend previous:</strong> dp[i] = dp[i-1] + arr[i]<br/>
            Pick whichever is larger!
          </div>

          <h3 style="color:var(--accent);margin:14px 0 6px;font-size:.95rem">Why It's DP</h3>
          <div class="box">
            <strong>Optimal Substructure:</strong> Best subarray ending at i depends on best subarray ending at i-1<br/>
            <strong>Overlapping Subproblems:</strong> dp[i] reuses dp[i-1]<br/>
            <strong>Memoization:</strong> Store results in dp table
          </div>

          <h3 style="color:var(--accent);margin:14px 0 6px;font-size:.95rem">Example Walkthrough</h3>
          <div class="box">
            arr = [-2, 1, -3, 4, -1, 2, 1, -5, 4]<br/>
            dp[0] = -2 (start)<br/>
            dp[1] = max(1, -2+1) = 1 (start fresh)<br/>
            dp[2] = max(-3, 1-3) = -2 (continue)<br/>
            dp[3] = max(4, -2+4) = 4 (start fresh)<br/>
            ... Result: 6 at [4,-1,2,1]
          </div>

          <h3 style="color:var(--accent);margin:14px 0 6px;font-size:.95rem">Complexity Comparison</h3>
          <div class="box">
            <strong>Brute Force:</strong> O(n²) — all subarrays<br/>
            <strong>Kadane's (Optimized DP):</strong> O(n) time, O(1) space<br/>
            <strong>DP (This):</strong> O(n) time, O(n) space (explicit table)
          </div>
        </div>
      </div>
    </main>
  ...
  <script>
    ...
    async function runDP() {
      ...
      for (let i = 1; i < n; i++) {
        ...
        const takeNew = state.arr[i];
        const extendPrev = state.dp[i - 1] + state.arr[i];
        state.dp[i] = Math.max(takeNew, extendPrev);

        updateDisplay();
        await sleep(150);

        state.currentLine = 7;
        if (state.dp[i] > state.maxSum) {
          state.maxSum = state.dp[i];
          state.maxIdx = i;
        }
        updateDisplay();
        await sleep(100);
      ...
      document.getElementById("metricStep").textContent = fmt(n);
      document.getElementById("metricDP").textContent = state.dp[n - 1];
    ...
    function updateDisplay() {
      ...
      let statsHtml = `<div class="step-explanation">
        <strong>Index [${state.idx}]:</strong> Element = ${state.arr[state.idx]}<br/>
        ${state.idx > 0 ? `DP[${state.idx}] = max(${state.arr[state.idx]}, ${state.dp[state.idx - 1]}+${state.arr[state.idx]}) = ${state.dp[state.idx] || '?'}<br/>` : `DP[0] = ${state.arr[0]}<br/>`}
        <span style="color:var(--good)">Max: ${state.maxSum}</span>
      </div>`;
    ...
    function renderArrayAndDP() {
      // Array
      let arrHtml = '';
      state.arr.forEach((val, i) => {
        let classes = 'arr-elem';
        if (i === state.idx) classes += ' active';
        if (val < 0) classes += ' negative';
        arrHtml += `<div class="${classes}" title="arr[${i}]=${val}">${val}</div>`;
      });
      document.getElementById("arrayDisplay").innerHTML = arrHtml;

      // DP Table
      let dpHtml = '';
      state.arr.forEach((_, i) => {
        let classes = 'dp-cell';
        if (state.dp[i] !== undefined) {
          if (i === state.idx) {
            classes += ' active';
          } else {
            classes += ' computed';
        ...
        const val = state.dp[i] !== undefined ? state.dp[i] : '?';
        dpHtml += `<div class="${classes}" title="dp[${i}]=${val}}">${val}</div>`;
      });
      document.getElementById("dpDisplay").innerHTML = dpHtml;
    }
    ...
    function reset() {
      ...
      document.getElementById("metricDP").textContent = "0";
    ...
    function applyCustom() {
      try {
        ...
        if (arr.length === 0 || arr.length > 20) throw "Array length: 1-20";
        state.arr = arr;
