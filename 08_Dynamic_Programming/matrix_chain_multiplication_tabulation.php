...
<head>
  ...
  <title>Matrix Chain Multiplication - Tabulation</title>
  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .dp-table{display:inline-grid;gap:2px;background:var(--border);padding:4px;border-radius:8px;margin:8px 0}
    .dp-row{display:grid;gap:2px}
    .dp-cell{width:36px;height:36px;display:flex;align-items:center;justify-content:center;border-radius:4px;font-size:.58rem;font-weight:700;background: var(--surface);color:var(--muted);border:1px solid var(--border)}
...
<body>
  <header>
    <div>
      <h1><span>Matrix Chain</span> Multiplication - Tabulation</h1>
      <p>Bottom-up DP - build solution iteratively by filling a table systematically</p>
    </div>
    <div class="actions">
    ...
    <aside class="left">
      ...
      <section class="sec">
        ...
        <div class="metrics">
          ...
          <div class="metric"><div class="metric-label">Computed</div><div class="metric-value" id="metricComputed">0</div></div>
      ...
      <section class="sec">
        ...
        <div class="box" style="text-align:center;font-size:.8rem;line-height:1.6">
          <div style="color:var(--accent);font-weight:700">O(n³)</div>
          <div style="color:var(--muted);margin-top:4px">Time: 3 nested loops</div>
        </div>
      </section>
    ...
    <main class="work">
      ...
      <div class="panel active" id="tab-visualizer">
        <div class="viz-container">
          ...
          <div class="viz-box">
            <div class="viz-head">📊 DP Table (i,j)</div>
      ...
      <div class="panel" id="tab-learn">
        <div style="overflow-y:auto;padding-right:10px">
          <h3 style="color:var(--accent);margin:10px 0 6px;font-size:.95rem">How Tabulation Works</h3>
          <div class="box">
            <strong>Build table iteratively!</strong> We compute the DP table row by row (or chain length by length). Each cell [i,j] represents the cost to multiply matrices i through j.
          </div>
          <h3 style="color:var(--accent);margin:14px 0 6px;font-size:.95rem">Bottom-Up Approach</h3>
          <div class="box">
            <strong>Iterative, no recursion!</strong><br/>
            • No function call overhead<br/>
            • Predictable order of computation<br/>
            • Cache-friendly (sequential access)<br/>
            • O(n³) guaranteed - 3 nested loops
          </div>
          <h3 style="color:var(--accent);margin:14px 0 6px;font-size:.95rem">Watch the Table Build</h3>
          <div class="box">
            The table fills diagonally: first all chains of length 2, then length 3, etc. Each cell shows the minimum cost to multiply that chain. Bottom-right cell = final answer!
          </div>
        </div>
      </div>
    </main>
  ...
  <script>
    ...
    const state = {
      dims: [10, 30, 5, 60], n: 0, result: 0, step: 0, computed: 0,
    ...
    async function runTabulation() {
      ...
      for (let len = 2; len <= n; len++) {
        ...
        for (let i = 1; i <= n - len + 1; i++) {
          ...
          for (let k = i; k < j; k++) {
            currentLine = 7;
            const left = state.table[i][k];
            updateDisplay(currentLine);
            await sleep(50);

            currentLine = 8;
            const right = state.table[k + 1][j];
            updateDisplay(currentLine);
            await sleep(50);

            currentLine = 9;
            const mult = state.dims[i - 1] * state.dims[k] * state.dims[j];
            updateDisplay(currentLine);
            await sleep(50);
            ...
            currentLine = 11;
            if (cost < state.table[i][j]) {
              currentLine = 12;
              state.table[i][j] = cost;
              state.computed++;
            }
            updateDisplay(currentLine);
            await sleep(60);
      ...
      document.getElementById("metricStep").textContent = fmt(state.step);
      document.getElementById("metricComputed").textContent = fmt(state.computed);
      ...
      if (state.n > 0) {
        for (let i = 1; i <= state.n; i++) {
          ...
          for (let j = 1; j <= state.n; j++) {
            ...
            const isComputed = val !== null && val !== Infinity;
            const cellClass = isCurrent ? 'current' : isComputed ? 'computed' : '';
            const display = val === null ? '—' : val === Infinity ? '∞' : fmt(val);
            rowHtml += `<div class="dp-cell ${cellClass}" title="[${i},${j}]=${display}">${i},${j}</div>`;
          }
          rowHtml += '</div>';
          tableHtml += rowHtml;
        }
      }
      document.getElementById("tableArea").innerHTML = tableHtml || '<div style="color:var(--muted)">Table will appear here</div>';
    }
    ...
    function reset() {
      ...
      document.getElementById("metricComputed").textContent = "0";
      ...
      document.getElementById("tableArea").innerHTML = '<div style="color:var(--muted)">Table will appear here</div>';
    }
