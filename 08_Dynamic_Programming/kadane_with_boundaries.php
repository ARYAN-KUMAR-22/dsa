...
<head>
  ...
  <title>Kadane's Algorithm - Max Subarray with Boundaries</title>
  ...
  <style>
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .boundary-label{position:absolute;top:-20px;font-size:.55rem;font-weight:800;color:var(--accent);white-space:nowrap}
...
<body>
  <header>
    <div>
      <h1><span>Kadane's Algorithm</span> - With Subarray Boundaries</h1>
      <p>Track start/end indices to find exactly which elements form the maximum subarray</p>
    </div>
    <div class="actions">
      <a class="primary" href="../index.php">Home</a>
      <a href="max_subarray_brute_force.php">Brute Force</a>
      <a href="max_subarray_sum.php">Kadane's Basic</a>
  ...
  <div class="app">
    <aside class="left">
      ...
      <section class="sec">
        ...
        <div class="metrics">
          <div class="metric"><div class="metric-label">Current Sum</div><div class="metric-value" id="metricCurrent">0</div></div>
          <div class="metric"><div class="metric-label">Max Sum</div><div class="metric-value" id="metricMax">0</div></div>
          <div class="metric"><div class="metric-label">Range</div><div class="metric-value" id="metricRange">[—,—]</div></div>
    ...
    <main class="work">
      <div class="tabs">
        <button class="tab active" data-tab="visualizer">🔍 Kadane with Boundaries</button>
      ...
      <div class="panel active" id="tab-visualizer">
        <div class="viz-container">
          <div class="viz-box">
            <div class="viz-head">💡 Tracking Logic</div>
            <div class="viz-content" id="codeArea"></div>
          </div>
          <div class="viz-box">
            <div class="viz-head">📊 Array with Boundaries</div>
            <div class="viz-content" style="display:flex;flex-direction:column;gap:10px;padding-top:20px">
      ...
      <div class="panel" id="tab-learn">
        <div style="overflow-y:auto;padding-right:10px">
          <h3 style="color:var(--accent);margin:10px 0 6px;font-size:.95rem">Enhanced Kadane's with Boundaries</h3>
          <div class="box">
            Kadane's algorithm finds the maximum sum in O(n), but what if you also need to know <strong>which elements</strong> form that subarray? This version tracks start and end indices!
          </div>

          <h3 style="color:var(--accent);margin:14px 0 6px;font-size:.95rem">Key Additions</h3>
          <div class="box">
            <strong>Track 4 pointers:</strong><br/>
            • currentStart: where current sum began<br/>
            • maxStart, maxEnd: boundaries of best subarray<br/>
            • Update boundaries when new max found
          </div>

          <h3 style="color:var(--accent);margin:14px 0 6px;font-size:.95rem">Logic Flow</h3>
          <div class="box">
            1. If current_sum becomes 0 → reset currentStart<br/>
            2. If current_sum exceeds max → update max AND update boundaries<br/>
            3. Display exactly which elements form maximum<br/>
            4. Also show the actual subarray sum
          </div>

          <h3 style="color:var(--accent);margin:14px 0 6px;font-size:.95rem">Practical Example</h3>
          <div class="box">
            Array: [-2, 1, -3, 4, -1, 2, 1, -5, 4]<br/>
            Max Sum: 6<br/>
            <span style="color:var(--good)">Subarray: [4, -1, 2, 1] (indices 3-6)</span><br/>
            This visualization shows elements 3,4,5,6 highlighted
          </div>

          <h3 style="color:var(--accent);margin:14px 0 6px;font-size:.95rem">Why This Matters</h3>
          <div class="box">
            • Interview question: often asks for boundaries too<br/>
            • Real-world: need to know WHICH elements, not just sum<br/>
            • Extends algorithm: minimal overhead for big benefit<br/>
  ...
  <script>
    ...
    const codeLines = [
      ...
      "      currentSum = 0",
      "      currentStart = i + 1",
      "  return maxSum, arr[maxStart:maxEnd+1]"
    ];
    ...
    async function runKadane() {
      ...
      for (let i = 0; i < state.arr.length; i++) {
        ...
        if (state.currentSum > state.maxSum) {
          ...
          state.maxStart = state.currentStart;
          updateDisplay();
          await sleep(80);
        ...
        if (state.currentSum < 0) {
          ...
          state.currentLine = 14;
          state.currentStart = i + 1;
          updateDisplay();
          await sleep(100);
        }
      }
      ...
      document.getElementById("metricCurrent").textContent = state.currentSum;
      document.getElementById("metricMax").textContent = state.maxSum;
      document.getElementById("metricRange").textContent = `[${state.maxStart},${state.maxEnd}]`;
      document.getElementById("metricTime").textContent = elapsed + "ms";
      
      const subarray = state.arr.slice(state.maxStart, state.maxEnd + 1);
      document.getElementById("subarrayBox").textContent = `[${subarray.join(', ')}]`;
    ...
    function updateDisplay() {
      ...
      let statsHtml = `<div class="step-explanation">
        ...
        Current Sum: ${state.currentSum} | Max: ${state.maxSum}<br/>
        ${state.maxStart <= state.maxEnd ? `<span style="color:var(--good)">✓ Best Range: [${state.maxStart}...${state.maxEnd}] = [${state.arr.slice(state.maxStart, state.maxEnd + 1).join(', ')}]</span>` : 'Searching...'}
      </div>`;
    ...
    function renderArray() {
      let html = '<div class="array-viz" style="position:relative;margin-top:30px">';
      state.arr.forEach((val, i) => {
        let classes = 'arr-elem';
        
        if (i === state.idx) {
          classes += ' active';
        } else if (i >= state.maxStart && i <= state.maxEnd) {
          classes += ' max';
        }
        ...
        let label = '';
        if (i === state.maxStart && state.maxStart <= state.maxEnd) {
          label = `<div class="boundary-label">START</div>`;
        }
        if (i === state.maxEnd && state.maxStart <= state.maxEnd) {
          label = `<div class="boundary-label">END</div>`;
        }
        
        html += `<div class="${classes}" title="arr[${i}]=${val}">${label}${val}</div>`;
      });
      html += '</div>';
    ...
    function reset() {
      ...
      document.getElementById("metricCurrent").textContent = "0";
